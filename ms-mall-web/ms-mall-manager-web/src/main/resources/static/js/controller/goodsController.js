...
app.controller('goodsController', function($scope, $controller, $location, goodsService,
	...
	$scope.$watch('entity.goods.typeTemplateId', function(newValue, oldValue) {
		...
		typeTemplateService.findOne(newValue).success(
				function(response) {
					...
					$scope.typeTemplate.brandIds = !$scope.typeTemplate.brandIds ? {} : JSON.parse($scope.typeTemplate.brandIds);// 品牌列表类型转换
	...
	$scope.updateStatusPass = function(){
		$scope.updateStatus('1');
	}
	
	$scope.updateStatusSendBack = function(){
	...
	//更新状态
	$scope.updateStatusByIdPass = function() {
		$scope.updateStatusById('1');
	}
	$scope.updateStatusByIdSendBack = function() {
		$scope.updateStatusById('2');
	}
	$scope.updateStatusById = function(status){
		goodsService.updateStatus( $scope.entity.goods.id ,status).success(
			function(response){
				if(response.success){
					$scope.reloadList();//刷新页面
					$scope.selectIds=[];
